/**
 * OceanEmbed-X Frontend Engine
 * This script runs the interactive ocean map dashboard.
 * 
 * What it does:
 * - Draws fast, smooth 60 FPS ocean colormaps on an HTML5 canvas
 * - Shows 2D satellite views (SST, SSS, winds, currents) and 15 underwater depth layers
 * - Renders interactive 3D depth sounding graphs when you click anywhere on the sea
 * - Works both online (connected to FastAPI) and offline (using built-in fluid physics)
 */

// Basic settings and tracking user state
const API_BASE = "http://localhost:8000/api/v1";
let isBackendOnline = false;

// The 15 standard ocean depths (in meters) we predict
const STANDARD_DEPTHS = [0, 5, 10, 20, 30, 50, 75, 100, 125, 150, 200, 300, 500, 700, 1000];
let currentDepthIndex = 10; // Start at 200m depth layer by default
let currentVariable = "temp"; // Current active map layer (e.g. temp, sst, sss, currents)
let currentActiveCoord = { lat: 15.0, lon: 70.0 }; // Last clicked location
let currentSynopticDay = 0; // Current simulation day (Day 1 to 10)
let isPlaying = false; // Timeline auto-playback toggle
let playInterval = null;
let playSpeed = 1; // Playback speed (1x, 2x, 5x)
...
/**
 * Offline Fluid Physics Engine
 * When running standalone in the browser (like on GitHub Pages), this calculates
 * natural, smooth ocean temperatures across the Arabian Sea and Bay of Bengal.
 * 
 * It uses smooth bell curves (Gaussian functions) and gradual blends (sigmoid functions)
 * so the ocean never has artificial square blocks, sharp cutoffs, or ugly seams.
 */
